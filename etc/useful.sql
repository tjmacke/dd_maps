...
-- SELECT stmts
-- select addresses w/geo to test that fwd canonicalization = bck canonicaliztion
PRAGMA foreign_keys = ON ;
SELECT as_reason, address, a_type, qry_address, rply_address
	FROM addresses
	WHERE as_reaons LIKE 'geo.ok.%' ;
...
-- get all addresses w/o first & last used dates
PRAGMA foreign_keys = ON ;
...
-- get the address_id for the src, dst addresses
PRAGMA foreign_keys = ON ;
SELECT  address_id
	FROM addresses
	WHERE address = A ;

-- get the dash_id's for this date; use script to select right one based on time_start, time_end
PRAGMA foreign_keys = ON ;
SELECT dash_id, time_start, time_end
	FROM dashes
	WHERE time_start LIKE 'Y:M:D' ;
...
-- update the date of last use
UPDATE addresses
	SET date_liast_use = D
	WHERE address = A ;

-- update the date of first use.  I added date_first_use, date_last_use after I started the table, so this was needed; shouldn't ever be used again
UPDATE addresses
